  ...
  ({ worldImage, storyAltText, imageCaption }: WorldImageCardProps) => {
    return (
      <figure className="relative shadow-xl shadow-black-500/50 overflow-hidden max-w-lg max-h-lg transition-all duration-300 cursor-pointer filter">
        ...
        <div className="absolute hover:opacity-0 inset-0 bg-black opacity-60" />
        <figcaption className="absolute px-4 text-lg text-white bottom-6 bg-[#FF7F50] rounded-r-md transition duration-75">
          <p>{imageCaption}</p>
        </figcaption>
      </figure>
    );
